...
class ChatBot:
    def __init__(self, update = "n"):
        ...
        #update = input("知识库是否需要更新(知识库文件有无改动)，输入y或n:\n")
        if update.lower() == 'y':
    ...
    def chat(self,query_str):
        ...
        if ('录入一下发票信息') in first_matched_doc['text']:
            ...
            path = input("我发现您需要读取发票文件, 我将调用工具录入数据, 请再次输入文件路径(比如: ./invoice_example/1.png): ")
            infor_titles, infors = get_invoice_info("./"+path)
            for (infor_title, infor) in zip(infor_titles, infors):
                response = response + "\n" + infor_title + ":" + infor
        elif ('发票信息全部录入') in first_matched_doc['text']:
            #response, history = inference(llm, template, query)
            path = input("我发现您需要读取全部发票文件并修改知识库, 请再次输入文件夹路径(比如: invoice_example): ")
            files = os.listdir("./" + path)
            with open("./ragData/invoice.txt", 'w') as f:
                f.write("{\n")
                for file in files:
                    infor_titles, infors = get_invoice_info("./" + path + "/" +file)
                    text = "\""
                    for (infor_title, infor) in zip(infor_titles, infors):
                        text = text + infor_title + ":" + infor + " "              
                    text += "\""
                    f.write(f"{text} : {text}")
                    f.write("\n\n")
                f.write("}\n")
            response = ("知识库文件添加成功，现在开始更新知识库。\n")
            docs = document_chunker(directory_path='./ragData',
                                    model_name=self.model_name,
                                    chunk_size=256)
            vec_store = create_vector_store(docs)
...
if __name__  == '__main__':
    ...
    print("=================================")
    while 1:
        #query_str = "请帮我录入一下发票信息，发票在./invoice_example/2.png"
        #query_str = "请你把指定文件夹中的全部发票文件录入到知识库中"
        #query_str = "江苏京东海元贸易有限公司的发票价税合计是多少?"
        query_str = input("user: ")
        res = chat.chat(query_str)
        print(res)
        print("=================================")
